    ...
    <div class="page-container">
      <div class="quiz-config">
        <div class="quiz-config-header">
          ...
          <p>Get clear, detailed explanations on any topic — dynamically tailored to your grade and learning style.</p>
        </div>

        <div class="quiz-config-form">
          <div class="form-group animate-item">
            <label>Topic</label>
            <input type="text" class="input-field" id="explain-topic" placeholder="e.g., Blood type, Photosynthesis, Derivatives, World War II" value="${initialTopic}">
          </div>
          ...
          <div class="quiz-config-actions animate-item">
            <button class="btn btn-secondary" id="explain-back-btn">Cancel</button>
            <button class="btn btn-primary btn-lg" id="explain-go-btn">💡 Get Explanation</button>
          </div>
        </div>
        ...
        <div id="explanation-result" style="display: none; margin-top: var(--space-8);">
          ...
          <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-xl); padding: var(--space-6); margin-top: var(--space-6); box-shadow: var(--shadow-md);">
            <div style="display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); margin-bottom: var(--space-4);">
              <div style="display: flex; align-items: center; gap: var(--space-3);">
                ...
                <div>
                  <span style="font-weight: 700; font-size: 1rem; color: var(--color-text);">DHRYZN explains:</span>
                  <div id="explanation-badge-sub" style="font-size: 0.78rem; color: var(--color-text-secondary);">Powered by Gemini 3.6 Flash</div>
                </div>
              </div>
              <button class="btn btn-secondary btn-sm" id="copy-explanation-btn" title="Copy explanation text" style="padding: 4px 10px; font-size: 0.8rem;">📋 Copy</button>
            </div>
          ...
          <div style="display: flex; gap: var(--space-3); margin-top: var(--space-5); flex-wrap: wrap;">
            <button class="btn btn-secondary" id="followup-quiz">📝 Generate Quiz on This</button>
            <button class="btn btn-secondary" id="followup-exam">🎯 Practice Exam</button>
            <button class="btn btn-secondary" id="followup-another">💡 Explain Another Topic</button>
          </div>
        </div>
      </div>
    </div>
  ...
  container.querySelector('#explain-go-btn').addEventListener('click', async () => {
    ...
    contentEl.innerHTML = `
      <div style="display: flex; align-items: center; gap: var(--space-3); padding: var(--space-4) 0; color: var(--color-text-secondary);">
        ...
        <span style="font-size: 0.9rem;">Gemini 3.6 Flash is crafting your explanation...</span>
      </div>
    `;
    ...
    try {
      ...
      contentEl.innerHTML = `<div style="color: var(--color-error);">Failed to generate explanation. Please check your connection and try again.</div>`;
    } finally {
