...
export class Store {
  constructor() {
    ...
    if (this.state && this.state.settings && this.state.settings.language) {
  ...
  set(key, value) {
    ...
    if (key === 'settings' && value && value.language) {
  ...
  update(key, updater) {
    ...
    if (key === 'settings' && this.state.settings && this.state.settings.language) {
      setLanguage(this.state.settings.language);
    }
    if (key === 'quizHistory' || key === 'examHistory' || key === 'subjects') {
      this.recalculateDynamicStats();
  ...
  addQuizToHistory(quiz) {
    ...
    const quizWithDate = {
      ...
      answers: quiz.answers ? JSON.parse(JSON.stringify(quiz.answers)) : []
    };
  ...
  addExamToHistory(exam) {
    ...
    const examWithDate = {
      ...
      answers: exam.answers ? JSON.parse(JSON.stringify(exam.answers)) : []
    };
