...
export async function generateTopicExplanation(topic, style = 'Quick Summary', gradeLevel = 'Grade 10') {
  ...
  const isIndonesian = lang === 'Indonesian';
...
Always tailor explanations to the requested target level (${gradeLevel}) and style (${style}).${isIndonesian ? '\nCRITICAL LANGUAGE REQUIREMENT: Write the ENTIRE explanation in Bahasa Indonesia (Indonesian).' : ''}
...
3. RANDOMIZE CORRECT ANSWER POSITIONS:
   - For Multiple Choice, evenly and randomly distribute correct answers across options A, B, C, and D (indices 0, 1, 2, and 3). Do NOT always place the correct answer at index 0.${isIndonesian ? '\n4. LANGUAGE REQUIREMENT: Write all question text, options, sample answers, and explanations in Bahasa Indonesia (Indonesian).' : ''}
...
[
  ...
  // Domain-Aware Intelligent Fallback Generator (Emergency only)
