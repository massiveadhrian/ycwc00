          ...
          <div class="chart-container">
            ...
            ` : `
              <div style="height: 160px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: var(--color-text-secondary); text-align: center; gap: var(--space-3);">
                <div>${t('progress.emptyTrend')}</div>
                <button class="btn btn-secondary btn-sm" id="progress-take-exam-btn">${t('progress.startPracticeExam')}</button>
              </div>
            `}
          </div>
        </div>
...
function getReadinessStatus(percent, hasData) {
  if (!hasData) return t('progress.emptyReadiness');
...
function getRecommendation(progress) {
  ...
  if (progress.totalQuestions === 0) {
    return {
      title: t('progress.firstQuizTitle'),
      desc: t('progress.firstQuizDesc'),
      route: 'quiz'
    };
  }
  return {
