...
export function renderHistory(container, params, store, router) {
  ...
  function openHistoryDetail(item) {
    console.log('[DHRYZN Runtime Diagnostic 4: targetItem in openHistoryDetail]', {
      id: item ? item.id : null,
      type: item ? item.type : null,
      hasQuestions: Boolean(item && item.questions),
      questionsIsArray: Boolean(item && Array.isArray(item.questions)),
      questionsLength: item && item.questions ? item.questions.length : 0,
      answersLength: item && item.answers ? item.answers.length : 0,
      item
    });
    ...
    const notAnsweredText = t('historyModal.notAnswered');
    ...
    if (item.questions && Array.isArray(item.questions) && item.questions.length > 0) {
      ...
      questionsHtml = `
        <div style="margin-top: var(--space-6);">
          <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: var(--space-4); flex-wrap: wrap; gap: var(--space-2);">
            <h3 style="font-size: var(--text-h3); font-weight: var(--weight-semibold); margin: 0;">${t('historyModal.questionReview')}</h3>
            <div style="display: flex; gap: var(--space-2);">
              <button class="history-filter-pill active" id="filter-all-questions">${t('historyModal.all')} (${item.questions.length})</button>
              <button class="history-filter-pill" id="filter-mistakes-only">${t('historyModal.mistakes')} (${incorrectCount})</button>
            </div>
          </div>

          <div id="no-mistakes-notice" style="display: none; background: var(--color-success-bg); border: 1px solid rgba(52, 211, 153, 0.2); border-radius: var(--radius-lg); padding: var(--space-4); text-align: center; margin-top: var(--space-3); color: var(--color-success); font-weight: 500;">
            ${t('historyModal.noMistakes')}
          </div>
          ...
          <div class="quiz-review-list" id="modal-questions-container" style="margin-top: 0;">
            ${item.questions.map((q, i) => {
              let isCorrectAnswer;
              if (isTF) {
                isCorrectAnswer = answers[i] !== null && answers[i] !== undefined && Boolean(answers[i]) === Boolean(q.answer);
              } else if (isText) {
                isCorrectAnswer = answers[i] && String(answers[i]).trim().length > 0;
              } else {
                isCorrectAnswer = answers[i] !== null && answers[i] !== undefined && q.correct !== undefined && Number(answers[i]) === Number(q.correct);
              }

              const userAnswerDisplay = isText
                ? (answers[i] || notAnsweredText)
                : isTF
                ? (answers[i] === null || answers[i] === undefined ? notAnsweredText : (answers[i] ? trueText : falseText))
                : (answers[i] !== null && answers[i] !== undefined && q.options && q.options[answers[i]] !== undefined ? q.options[answers[i]] : notAnsweredText);
                ...
                <div class="quiz-review-item ${isCorrectAnswer ? 'correct' : 'incorrect'}" data-is-correct="${isCorrectAnswer}">
                  <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: var(--space-2);">
                    ...
                    <span class="badge" style="background: ${isCorrectAnswer ? 'var(--color-success-bg)' : 'var(--color-error-bg)'}; color: ${isCorrectAnswer ? 'var(--color-success)' : 'var(--color-error)'}; font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; flex-shrink: 0; margin-left: 8px;">
                      ${isCorrectAnswer ? t('historyModal.correctBadge') : t('historyModal.missedBadge')}
                    </span>
                  </div>
                  <div class="quiz-review-answer" style="margin-bottom: var(--space-2); line-height: 1.5;">
                    <div><strong style="color: var(--color-text);">${t('historyModal.yourAnswer')}</strong> <span style="${!isCorrectAnswer ? 'color: var(--color-error); font-weight: 500;' : ''}">${escapeHtml(userAnswerDisplay)}</span></div>
                    ${!isCorrectAnswer ? `<div><strong style="color: var(--color-text);">${t('historyModal.correctAnswer')}</strong> <span style="color: var(--color-success); font-weight: 500;">${escapeHtml(correctAnswerDisplay)}</span></div>` : ''}
                  </div>
                  ${q.explanation ? `
                    <div style="font-size: var(--text-xs); color: var(--color-text-secondary); background: var(--color-bg); padding: var(--space-2) var(--space-3); border-radius: var(--radius-md); border-left: 2px solid var(--color-primary-light);">
                      ${t('historyModal.explanation')} ${escapeHtml(q.explanation)}
                    </div>
                  ` : ''}
                </div>
              `;
      ...
      questionsHtml = `
        <div style="background: var(--color-bg); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-5); margin-top: var(--space-5); text-align: center;">
          ...
          <h4 style="margin-bottom: var(--space-1); font-weight: var(--weight-semibold);">${t('historyModal.performanceSummary')}</h4>
          <p style="color: var(--color-text-secondary); font-size: var(--text-sm); line-height: var(--leading-relaxed); margin: 0 auto; max-width: 420px;">
            ${getLanguage() === 'Indonesian'
              ? `Kamu menyelesaikan ${isQuiz ? 'kuis' : 'latihan ujian'} ini dengan akurasi <strong>${item.score}%</strong>, menjawab <strong>${item.correct} dari ${item.total}</strong> pertanyaan dengan benar dalam <strong>${item.duration}</strong>.`
        ...
        <div class="history-modal-body" id="history-modal-body">
          <div class="history-modal-stats">
            <div class="history-modal-stat-card">
              ...
              <div class="history-modal-stat-label">${t('historyModal.accuracy')}</div>
            </div>
            <div class="history-modal-stat-card">
              <div class="history-modal-stat-value" style="color: var(--color-success);">${item.correct}</div>
              <div class="history-modal-stat-label">${t('historyModal.correctLabel')}</div>
            </div>
            <div class="history-modal-stat-card">
              <div class="history-modal-stat-value" style="color: var(--color-error);">${incorrectCount}</div>
              <div class="history-modal-stat-label">${t('historyModal.incorrectLabel')}</div>
            </div>
          </div>

          <div class="history-modal-meta-row">
            <span>📅 <strong>${t('historyModal.dateLabel')}</strong> ${formatDate(item.date)}</span>
            <span>⏱ <strong>${t('historyModal.timeLabel')}</strong> ${item.duration}</span>
            <span>📝 <strong>${t('historyModal.totalQuestionsLabel')}</strong> ${item.total} ${t('historyModal.questions')}</span>
          </div>

          ${questionsHtml}
        </div>
        <div class="history-modal-footer">
          <button class="btn btn-secondary" id="history-modal-dismiss-btn">${t('historyModal.close')}</button>
          <button class="btn btn-primary" id="history-modal-retake-btn">${isQuiz ? t('historyModal.retakeQuiz') : t('historyModal.retakeExam')}</button>
        </div>
      </div>
    `;
    ...
    const noMistakesEl = detailModal.querySelector('#no-mistakes-notice');

    if (btnAll && btnMistakes && qContainer) {
      btnAll.onclick = () => {
        btnAll.classList.add('active');
        btnMistakes.classList.remove('active');
        if (noMistakesEl) noMistakesEl.style.display = 'none';
      ...
      btnMistakes.onclick = () => {
        ...
        if (noMistakesEl) {
          noMistakesEl.style.display = mistakeCount === 0 ? 'block' : 'none';
        }
      };
    }
...
function formatDate(dateStr) {
  ...
  const locale = getLanguage() === 'Indonesian' ? 'id-ID' : 'en-US';
